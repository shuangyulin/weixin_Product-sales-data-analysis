...
def upload_csv_mapreduce_hadoop():
    # 查询数据
    query = "SELECT * FROM zhibodaihuo"
    df = pd.read_sql(query, connection)
    local_csv_path = os.path.join(parent_directory,"zhibodaihuo.csv")
    # 导出为 CSV 文件
    df.to_csv(local_csv_path, index=False)
    print(f"数据成功导出到 CSV 文件: {local_csv_path}")
    hdfs_csv_path = '/input/zhibodaihuo.csv'
    # 目标 HDFS 路径
    ...
    df = pd.read_sql(query, connection)
    local_csv_path = os.path.join(parent_directory,"orders.csv")
    # 导出为 CSV 文件
    df.to_csv(local_csv_path, index=False)
    print(f"数据成功导出到 CSV 文件: {local_csv_path}")
    hdfs_csv_path = '/input/orders.csv'
    # 目标 HDFS 路径
    if hadoop_client.status(hdfs_csv_path,strict=False):
        # 删除HDFS上的文件
        hadoop_client.delete(hdfs_csv_path)
...
def send_cmd():
    ...
    [
        ...
        "-mapper", f"\"python value_mapper.py {csv_index('zhibodaihuo.csv','title')} {csv_index('zhibodaihuo.csv','duration')} 无  \"",
        "-reducer", "\"python value_reducer.py title\"",
        "-input", "hdfs://localhost:9000/input/zhibodaihuo.csv",
        "-output", "hdfs://localhost:9000/output/zhibodaihuo/valuetitleduration"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/value_mapper.py,hdfs://localhost:9000/input/value_reducer.py\"",
    ...
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/value_mapper.py,hdfs://localhost:9000/input/value_reducer.py\"",
        "-mapper", f"\"python value_mapper.py {csv_index('zhibodaihuo.csv','title')} {csv_index('zhibodaihuo.csv','collectcount')} 无  \"",
        "-reducer", "\"python value_reducer.py title\"",
        "-input", "hdfs://localhost:9000/input/zhibodaihuo.csv",
        "-output", "hdfs://localhost:9000/output/zhibodaihuo/valuetitlecollectcount"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/value_mapper.py,hdfs://localhost:9000/input/value_reducer.py\"",
        "-mapper", f"\"python value_mapper.py {csv_index('zhibodaihuo.csv','title')} {csv_index('zhibodaihuo.csv','commentcount')} 无  \"",
        "-reducer", "\"python value_reducer.py title\"",
        "-input", "hdfs://localhost:9000/input/zhibodaihuo.csv",
        "-output", "hdfs://localhost:9000/output/zhibodaihuo/valuetitlecommentcount"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/value_mapper.py,hdfs://localhost:9000/input/value_reducer.py\"",
        "-mapper", f"\"python value_mapper.py {csv_index('zhibodaihuo.csv','title')} {csv_index('zhibodaihuo.csv','diggcount')} 无  \"",
        "-reducer", "\"python value_reducer.py title\"",
        "-input", "hdfs://localhost:9000/input/zhibodaihuo.csv",
        "-output", "hdfs://localhost:9000/output/zhibodaihuo/valuetitlediggcount"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/value_mapper.py,hdfs://localhost:9000/input/value_reducer.py\"",
        "-mapper", f"\"python value_mapper.py {csv_index('zhibodaihuo.csv','nickname')} {csv_index('zhibodaihuo.csv','diggcount')} 无  \"",
        "-reducer", "\"python value_reducer.py nickname\"",
        "-input", "hdfs://localhost:9000/input/zhibodaihuo.csv",
        "-output", "hdfs://localhost:9000/output/zhibodaihuo/valuenicknamediggcount"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/value_mapper.py,hdfs://localhost:9000/input/value_reducer.py\"",
        "-mapper", f"\"python value_mapper.py {csv_index('zhibodaihuo.csv','nickname')} {csv_index('zhibodaihuo.csv','sharecount')} 无  \"",
        "-reducer", "\"python value_reducer.py nickname\"",
        "-input", "hdfs://localhost:9000/input/zhibodaihuo.csv",
        "-output", "hdfs://localhost:9000/output/zhibodaihuo/valuenicknamesharecount"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/group_mapper.py,hdfs://localhost:9000/input/group_reducer.py\"",
        "-mapper", f"\"python group_mapper.py {csv_index('zhibodaihuo.csv','title')}\"",
        "-reducer", "\"python group_reducer.py title\"",
        "-input", "hdfs://localhost:9000/input/zhibodaihuo.csv",
        "-output", "hdfs://localhost:9000/output/zhibodaihuo/grouptitle"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/group_mapper.py,hdfs://localhost:9000/input/group_reducer.py\"",
        "-mapper", f"\"python group_mapper.py {csv_index('zhibodaihuo.csv','nickname')}\"",
        "-reducer", "\"python group_reducer.py nickname\"",
        "-input", "hdfs://localhost:9000/input/zhibodaihuo.csv",
        "-output", "hdfs://localhost:9000/output/zhibodaihuo/groupnickname"
    ],
    ]
